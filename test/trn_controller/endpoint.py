...
class endpoint:
    ...
    def __init__(self, vni, netid, ip, prefixlen, gw_ip, host, netip, tuntype='gnv', bridge='br0', host_ep=False):
        ...
        self.iperf_report = f'''{self.output_dir}/{self.ns}_{self.ip}_iperf'''
        self.udp_sent_file = f'''{self.output_dir}/{self.ns}_{self.ip}_udp_sent'''

        self.ns_exec = f'''sudo ip netns exec {self.ns}'''
    ...
    def do_iperf_serve(self, report_suffix='', affinity='1', args=''):
        ...
        cmd = f'''{self.bash_cmd} '(taskset -c {affinity} iperf -s {args} > {report} 2>&1) &' '''
        self.host.run(cmd)
